...
test.describe('Default Recipients', () => {
  test('[DEFAULT_RECIPIENTS]: default recipients are added to documents created via UI', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').last().fill('regular-signer@vedasign.uk');
    await page
    ...
    await expect(async () => {
      ...
      const regularSigner = envelope.recipients.find((r) => r.email === 'regular-signer@vedasign.uk');
  ...
  test.skip('[DEFAULT_RECIPIENTS]: default recipients cannot be removed from a document', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').first().fill('regular-signer@vedasign.uk');
    await page
      .getByPlaceholder(/Recipient/)
      .first()
      .fill('Regular Signer');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created via API have default recipients', async ({ request }) => {
    ...
    const payload: TCreateEnvelopePayload = {
      ...
      recipients: [
        {
          email: 'api-recipient@vedasign.uk',
          name: 'API Recipient',
    ...
    const apiRecipient = envelope.recipients.find((r) => r.email === 'api-recipient@vedasign.uk');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created from template have default recipients', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email').fill('template-recipient@vedasign.uk');
    ...
    const templateRecipient = document.recipients.find((r) => r.email === 'template-recipient@vedasign.uk');

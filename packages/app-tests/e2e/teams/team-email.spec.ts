...
test('[TEAMS]: send team email request', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. Legal').fill('test@test.vedasign.uk');
  await page.getByPlaceholder('example@example.com').click();
  await page.getByPlaceholder('example@example.com').fill('test@test.vedasign.uk');
...
test('[TEAMS]: accept team email request', async ({ page }) => {
  ...
  const teamEmailVerification = await seedTeamEmailVerification({
    email: `team-email-verification--${team.url}@test.vedasign.uk`,
    teamId: team.id,
  });
